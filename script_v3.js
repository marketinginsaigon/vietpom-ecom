/* ============================================================
   BỘ MÃ LOGIC GIỎ HÀNG VIETPOM - PHIÊN BẢN KHỬ CACHE TUYỆT ĐỐI V3
   ============================================================ */
            ...
            <div class="product-card" data-id="${p.id}">
                ...
                <div class="product-bottom">
                    ...
                    <div class="action-zone">
                        ${currentQty === 0 ? `
                            <button class="add-btn" onclick="window.updateCartItem('${p.id}', 1)">🛒 Chọn mua</button>
                        ` : `
                            <div class="qty-control">
                                <button onclick="window.updateCartItem('${p.id}', ${currentQty - 1})">-</button>
...
// 4. ÉP HÀM RA WINDOW TOÀN CỤC - TRIỆT LỖI DEFINED HOÀN TOÀN
...
function updateSingleProductUI(id, qty) {
    ...
    if (qty <= 0) {
        zone.innerHTML = `<button class="add-btn" onclick="window.updateCartItem('${id}', 1)">🛒 Chọn mua</button>`;
    } else {
        zone.innerHTML = `
            <div class="qty-control">
                <button onclick="window.updateCartItem('${id}', ${qty - 1})">-</button>
...
// 5. TỔNG HỢP GIỎ HÀNG VÀ TÍNH FREESHIP
        ...
        html += `
            <div class="cart-item" style="display:flex; justify-content:space-between; align-items:center; padding:12px 0; border-bottom:1px solid #e2e8f0;">
                ...
                <div style="display:flex; align-items:center; gap:8px; margin-right:15px; flex-shrink:0;">
                    <button onclick="window.updateCartItem('${id}', ${item.qty - 1})" style="width:24px; height:24px; background:#e2e8f0; border:none; border-radius:4px; font-weight:bold; cursor:pointer; color:#475569; display:flex; align-items:center; justify-content:center;">-</button>
                    <span style="font-weight:bold; font-size:14px; color:#15558D; min-width:16px; text-align:center;">${item.qty}</span>
                    <button onclick="window.updateCartItem('${id}', ${item.qty + 1})" style="width:24px; height:24px; background:#15558D; border:none; border-radius:4px; font-weight:bold; cursor:pointer; color:#fff; display:flex; align-items:center; justify-content:center;">+</button>
                </div>
...
// 7. SUBMIT FORM ĐƠN HÀNG VỀ SHEET
document.getElementById('orderForm')?.addEventListener('submit', async (e) => {
    ...
    formParams.append('total', `${finalTotalAmount}`); // Số nguyên thuần túy bảo vệ Tóm tắt Sheet
